...
export async function assessBranchWriteReconciliation(
  ...
  if (operation.state === "succeeded") {
    return baseAssessment(operation, "not_required", false, false);
  }

  if (!operation.operation) {
    return baseAssessment(operation, "legacy_operation_context_unavailable", true, false);
  }
  ...
  try {
    ...
    if (observedBranch) {
      return {
        ...baseAssessment(operation, "provider_branch_present", true, true),
        observedBranch,
      };
    }
    return baseAssessment(operation, "provider_branch_absent", true, true);
  } catch {
    return baseAssessment(operation, "provider_observation_unavailable", true, true);
  }
}
